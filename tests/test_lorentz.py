...
def test_expmap_finite_at_a_step_that_used_to_nan():
    m = LorentzManifold()
    torch.manual_seed(0)
    for r in (6.0, 14.0, 30.0, 46.0):
        x = torch.full((1, 64), math.sinh(r) / 8.0)
        for step in (149.8, 1e6):
            u = torch.full((1, 64), step / 8.0)
            assert torch.isfinite(m.expmap(x, u)).all()


def test_clamp_leaves_ordinary_steps_untouched():
    """Same arithmetic as expmap with the clamp removed: bit-identical below the bound."""
    m = LorentzManifold()
    torch.manual_seed(0)

    def unclamped(x, u):
        qq = m.inner(x, u, keepdim=True).clamp_min(0.0)
        tsq = qq * m._inv_k
        safe = qq > torch.finfo(qq.dtype).tiny
        nrm = torch.where(safe, qq, torch.ones_like(qq)).sqrt()
        t = nrm * m._inv_sqrt_k
        cosh_t = torch.where(safe, torch.cosh(t), 1.0 + tsq * 0.5)
        coef = torch.where(safe, torch.sinh(t) * (m._sqrt_k / nrm), 1.0 + tsq / 6.0)
        return torch.addcmul(cosh_t * x, coef, u)

    for r in (0.0, 1.0, 6.0, 14.0, 30.0):
        x = torch.full((1, 64), math.sinh(r) / 8.0)
        for scale in (0.0, 1e-6, 1e-3, 1.0, 40.0):
            u = torch.randn(1, 64) * scale
            ref = unclamped(x, u)
