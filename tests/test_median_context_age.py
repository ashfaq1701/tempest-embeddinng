...
def _call(**kw):
    return wt.median_context_age(None, torch.device("cpu"), torch.zeros(1), torch.zeros(1),
                                 max_walk_len=2, num_walks_per_node=1, **kw)
...
def test_excludes_seed_and_padding(monkeypatch):
    ...
    assert _call(min_tokens=1) == 20.0          # median(10,20,30); 0 and -1 would drag it down
...
def test_drops_zero_gap_ties(monkeypatch):
    ...
    assert _call(min_tokens=1) == 4.0           # the two age-0 ties are dropped first


def test_returns_none_when_too_cold(monkeypatch):
    ages = torch.tensor([[0, 7, -1, -1, -1]], dtype=torch.int64)
    mask = torch.tensor([[True, True, False, False, False]])
    seed = torch.tensor([[True, False, False, False, False]])
    _patch(monkeypatch, _FakeTokens(ages, mask, seed))
    assert _call(min_tokens=1000) is None       # caller must fall back, not trust one token
