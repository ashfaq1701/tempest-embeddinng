...
def _build_walks() -> tuple[WalkData, dict, int]:
    ...
    # Wiki edges are bipartite user→page action streams; the contract
    # test treats them as undirected (Tempest is always undirected).
    wg = WalkGenerator(
        use_gpu=False,
    ...
    # undirected: every edge is reachable in reverse too
    for s, t, time in zip(src, tgt, ts):
