...
For each walk, sample one positive `p` **uniformly** from valid walk positions `{n_0..n_{L-2}}`, one uniform-random negative `q`. Compute hinge `max(0, m − cos(a, p) + cos(a, q))` with **cosine similarity** (not raw dot product), margin `m = 0.5`. **Multiply the hinge by `w(p)` — the positional/temporal weight at the sampled position** — so Δt enters the loss the same way it does for InfoNCE and SGNS (consistency fix; see audit in §4.7.1 commentary). Semi-hard mining mask: only triplets with `pos_sim − neg_sim < m AND neg_sim < pos_sim` contribute gradient. **`η_uniform = 0`; add `weight_decay_emb = 1e-4`** to supply the norm control uniformity used to provide. One triplet per walk (over-sampling causes spurious gradient correlation). NEVER enable hard mining — bipartite graphs collapse under it.
