...
Hyperparameters exposed at CLI (and their grouping):
  ...
  Walks:          --embedding-num-walks-per-node, --embedding-max-walk-len,
                  ...
                  (Embedding + link-pred sides are both backward;
...
def parse_args() -> argparse.Namespace:
    ...
    # Walks.
    # *-num-walks-per-node is K walks per seed. Graphs are treated as
    # undirected: both the embedding side and the link-pred head draw
    # BACKWARD walks and spend the full K on them. Bias knobs are per
    # direction; defaults reflect Tempest's chronological semantics:
    ...
    p.add_argument(
        ...
        help="K walks per seed for the link-pred head. The head consumes "
             "BACKWARD walks (graphs are treated as undirected) and spends "
             "the full K on them.",
    )
    p.add_argument(
