...
def parse_args() -> argparse.Namespace:
    ...
    # Walk encoder (feature-flagged; baseline path runs unchanged when off).
    p.add_argument(
        "--use-walk-encoder", action="store_true",
        help="Enable the walk encoder. Per-seed h_seed (derived from "
             "Tempest walks via per-edge MLP + GRU + mean-pool + MLP_seed) "
             "replaces E[seed] at the InfoNCE seed projection AND at the "
             "link head. E lookups inside the encoder are detached so "
             "BCE does not reach E (Approach 2). Default OFF.",
    )
...
def main() -> Dict[str, Any]:
    ...
    # Edge feature dim is dataset-specific (None if absent). The walk
    # encoder needs this to size its per-edge MLP input.
    d_edge_feat = (
