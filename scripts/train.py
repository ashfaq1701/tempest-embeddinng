...
def main() -> Dict[str, Any]:
    ...
    n_Pcb = sum(p.numel() for p in trainer.p_context_backward.parameters())
    n_Pcf = sum(p.numel() for p in trainer.p_context_forward.parameters())
    n_H = sum(p.numel() for p in trainer.link_head.parameters())
    print(f"  embedding_table:    {n_E:>12,}")
    print(f"  p_target:           {n_Pt:>12,}")
    print(f"  p_context_backward: {n_Pcb:>12,}")
    print(f"  p_context_forward:  {n_Pcf:>12,}")
    print(f"  link_head:          {n_H:>12,}")
    print(f"  TOTAL trainable:    {n_E + n_Pt + n_Pcb + n_Pcf + n_H:>12,}")
