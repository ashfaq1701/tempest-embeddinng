#!/bin/bash
# Curated-archive sweep: all 8 TGB-Seq datasets at d64/K5/lr=3e-3, popularity channel ON.
#
# This machine produces replicate 3. Two other agents on other machines produce replicates 1
# and 2 into the same experiment_logs/complete_runs/d64_k5_lr3e-3/<dataset>/ directories.
#
...
ROOT=$WD/logs/archive_d64k5_lr3e3/d64_k5_lr3e-3
ARCHIVE=$WD/experiment_logs/complete_runs/d64_k5_lr3e-3
mkdir -p "$ROOT"
...
{
  echo "# ARCHIVE SWEEP  replicate=$REP  cell=d64_k5_lr3e-3  branch=$BRANCH commit=$SHA"
  [ -n "$DIRTY" ] && echo "# WARNING: uncommitted tracked changes -- SHA does not describe the code that ran"
  echo "# NOTE: the driver scripts/run_archive_d64k5_lr3e3.sh is UNTRACKED (kept out of git by request); the SHA describes the trained code only"
  echo "# d_emb=64 k_train=5 lr=3e-3 seed=42 patience 5 cap 100 --use-pop-bias"
...
--d-emb 64 --k-train 5 --lr 3e-3 --num-epochs 100 --early-stop-patience 5 --use-pop-bias \
$BIP --use-gpu --use-gpu-tempest $EXTRA"
  {
    echo "# dataset=$NAME ($EDGES edges, $([ -n "$BIP" ] && echo BIPARTITE || echo NON-bipartite))"
    echo "# d_emb=64 k_train=5 lr=3e-3 seed=42  patience 5  cap 100  --use-pop-bias"
    echo "# experiment=archive_d64k5_lr3e3 cell=d64_k5_lr3e-3 tag=run_$REP  (archived as ${REP}.log)"
    echo "# branch=$BRANCH commit=$SHA"
    [ -n "$DIRTY" ] && echo "# WARNING: uncommitted tracked changes -- SHA does not describe the code that ran"
    echo "# NOTE: driver scripts/run_archive_d64k5_lr3e3.sh is UNTRACKED; SHA covers the trained code only"
    echo "# replicate $REP of 3; replicates 1 and 2 are produced by other agents on other machines."
    echo "# part of the sweep driven by scripts/run_archive_d64k5_lr3e3.sh; see ../../DRIVER.log"
    echo "# started=$(date '+%F %T')"
    echo "# cmd: $CMD"
    echo
  } > "$LOG"
