#!/bin/bash
# Curated-archive sweep: all 8 TGB-Seq datasets at d64/K5 with the popularity channel ON.
#
# This machine produces replicate 3. Two other agents on other machines produce replicates 1
# and 2 into the same experiment_logs/other_geometries/poincare/d64_k5_lr1e-3/<dataset>/ directories.
#
...
ROOT=$WD/logs/archive_d64k5_popbias/d64_k5
ARCHIVE=$WD/experiment_logs/other_geometries/poincare/d64_k5_lr1e-3
mkdir -p "$ROOT"
...
{
  echo "# ARCHIVE SWEEP  replicate=$REP  cell=d64_k5  branch=$BRANCH commit=$SHA"
...
$BIP --use-gpu --use-gpu-tempest $EXTRA"
  {
    ...
    echo "# d_emb=64 k_train=5 lr=1e-3 seed=42  patience 5  cap 100  --use-pop-bias"
    echo "# experiment=archive_d64k5_popbias cell=d64_k5 tag=run_$REP  (archived as ${REP}.log)"
    echo "# branch=$BRANCH commit=$SHA"
    [ -n "$DIRTY" ] && echo "# WARNING: uncommitted tracked changes -- SHA does not describe the code that ran"
    echo "# replicate $REP of 3; replicates 1 and 2 are produced by other agents on other machines."
    echo "# part of the sweep driven by scripts/run_archive_d64k5_popbias.sh; see ../../DRIVER.log"
    echo "# started=$(date '+%F %T')"
    echo "# cmd: $CMD"
    echo
  } > "$LOG"
