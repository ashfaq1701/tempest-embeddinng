...
mkdir -p "$ROOT"
DRIVER=$ROOT/DRIVER_wikilink_yelp.log
...

{
  ...
  echo "# NOTE: driver scripts/run_lorentz_wikilink_yelp.sh is UNTRACKED; SHA covers the trained code only"
...
$BIP --use-gpu --use-gpu-tempest"
  {
    ...
    echo "# driven by scripts/run_lorentz_wikilink_yelp.sh (UNTRACKED); see ../DRIVER.log"
    echo "# started=$(date '+%F %T')"
    echo "# cmd: $CMD"
    echo
  } > "$LOG"
