...
Hyperparameters exposed at CLI (and their grouping):
  ...
  Optimisation:   --lr, --weight-decay, --batch-size, --eval-batch-size,
                  ...
                  (plain AdamW at a constant LR — no scheduler)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--weight-decay", default=0.0, type=float,
                   help="AdamW weight decay (default 0: E is L2-normalized, decay couples into eff. LR).")
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="After training, dump the best-val-restored embedding-table "
             ...
             "Rows are plain Euclidean embeddings (not unit-norm). "
             "Off by default.",
    )
