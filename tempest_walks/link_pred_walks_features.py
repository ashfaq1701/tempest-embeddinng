...
tensors LinkPredHeadV2 expects.
...
Notes:
  - The seed slot's `gap_norm` uses (t_query - t_min) / T_train rather
    than the sentinel. This gives the head a "where in train history is
    this seed" signal.
...
def make_head_inputs(
    ...
    # gap_norm:
    #   non-seed positions: (t_query - t_edge) / T_train  ∈ [0, 1]
    #   seed positions:     (t_query - t_min)  / T_train
    t_query = t_query_per_u.to(device).long().view(B, 1, 1).expand_as(nodes)
    raw_gap = (t_query - ts).clamp(min=0).float()
    gap_norm = raw_gap / max(float(T_train), 1.0)
    ...
    seed_gap_norm = (seed_gap / max(float(T_train), 1.0)).view(B, 1, 1)
    gap_norm = torch.where(
        seed_oh & valid, seed_gap_norm.expand_as(gap_norm), gap_norm,
    )
    gap_norm = gap_norm.clamp(0.0, 1.0)
