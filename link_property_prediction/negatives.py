...
Three flavours:
  ...
  - HistoricalNegativeSampler: per-source reservoir (Vitter R) of past
                              destinations mixed with a random fallback —
                              matches TGB eval's hist/rnd mix at train time.
                              A training-side ALTERNATIVE to Uniform (opt-in).
                              **NOT for tgbl-wiki**: on recurrence-dominated
                              datasets most eval positives ARE historical, so
                              training against them pushes E[u] away from the
                              eval signal and collapses MRR. Appropriate for
                              low-recurrence datasets (e.g. tgbl-review, ~8%
                              historical eval negatives — see CLAUDE.md TGB
                              stats). Requires the trainer to call `observe()`
                              post-scoring and `reset()` per epoch.
