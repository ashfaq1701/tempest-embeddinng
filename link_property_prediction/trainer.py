...
class Trainer:
    def __init__(self, config: TrainerConfig, device: Optional[torch.device] = None):
        ...
        # with the log-parameterised distance temperature split off at `lr_temperature`. Adam
        # moves a parameter by ~lr per step regardless of gradient size, so the temperature's step
        # size alone sets how fast the distance scale slews; at the shared lr it needed many epochs
        # to reach the scale Patent wants. Matches either form of the knob -- the linear `geo_temp`
        # or the log-parameterised `geo_temp_raw` -- and splits by identity, not name, so a head
        # carrying neither simply yields one group.
