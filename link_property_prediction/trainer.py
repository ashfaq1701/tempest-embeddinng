...
class Trainer:
    def __init__(self, config: TrainerConfig, device: Optional[torch.device] = None):
        ...
        # appears. The dense optimiser stepped and transported all N rows every batch.
        # SparseRiemannianAdam takes no `stabilize=` (its __init__ omits it); harmless,
        # since this chart's projx is the identity.
