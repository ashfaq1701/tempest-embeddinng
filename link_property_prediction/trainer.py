...
class Trainer:
    ...
    def _geometry_probe(self) -> Dict[str, float]:
        """Boundary watch: mean and max Euclidean norm of E.

        The Euclidean head leaves E unconstrained (initialised inside the unit
        ball, magnitude then free), so |E[v]| is the meaningful per-node
        magnitude -- the same quantity the pooler reads as its `norm` feature.
        Logged as r_mean/r_max.
        """
        r = self.model.E.weight.detach().norm(dim=-1)
