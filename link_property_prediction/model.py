...
class BagWeights(nn.Module):
    ...
    def forward(self, tokens: WalkTokens) -> torch.Tensor:
        ...
        d0_mid = self.geom.dist0(mid).unsqueeze(-1) \
            .clamp_min(_DENOM_FLOOR)                                         # [Q, 1]
        d0_seed = self.geom.dist0(x_seed).unsqueeze(-1) \
            .clamp_min(_DENOM_FLOOR)                                         # [Q, 1]
        r_mid = d_mid / d0_mid * m                                           # [Q, T]
        r_seed = d_seed / d0_seed * m                                        # [Q, T]
        q_mid = d0_tok / d0_mid * m                                          # [Q, T]
        q_seed = d0_tok / d0_seed * m                                        # [Q, T]

        feats = self._standardise(
            torch.stack([age, pos, r_mid, r_seed, q_mid, q_seed], dim=-1),
            valid).to(xt.dtype)                                              # [Q, T, 6]
